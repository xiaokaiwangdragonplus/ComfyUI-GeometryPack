...
const getBasePath = () => {
    try {
        ...
        console.log("[GeomPack BBox] getBasePath - import.meta.url:", jsUrl);
        ...
        if (jsMatch && jsMatch[1]) {
            console.log("[GeomPack BBox] getBasePath - extracted from import.meta.url:", jsMatch[1]);
            return jsMatch[1];
        }
        
        // Fallback to window.location.pathname
        const pathname = window.location.pathname;
        console.log("[GeomPack BBox] getBasePath - window.location.pathname:", pathname);
        const extensionsIndex = pathname.indexOf('/extensions/');
        if (extensionsIndex > 0) {
            const basePath = pathname.substring(0, extensionsIndex);
            console.log("[GeomPack BBox] getBasePath - extracted from pathname (extensions found):", basePath);
            return basePath;
        }
        ...
        if (pathname && pathname !== '/' && pathname.endsWith('/')) {
            ...
            console.log("[GeomPack BBox] getBasePath - extracted from pathname (trailing slash):", basePath);
            return basePath;
        }
        
        console.log("[GeomPack BBox] getBasePath - returning empty string");
        return '';
    } catch (e) {
        console.error("[GeomPack BBox] getBasePath - error:", e);
        return '';
    }
};
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "GeomPackPreviewBoundingBoxesVTK") {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                console.log("[GeomPack BBox] Base path:", basePath);
                const viewerUrl = `${basePath}/extensions/${EXTENSION_FOLDER}/viewer_vtk.html?v=` + Date.now();
                console.log("[GeomPack BBox] Setting initial iframe.src to:", viewerUrl);
